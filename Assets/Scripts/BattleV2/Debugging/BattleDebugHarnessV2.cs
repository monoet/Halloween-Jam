...
namespace BattleV2.Debugging
{
    ...
    public sealed class BattleDebugHarnessV2 : MonoBehaviour, IBattleInputProvider, ITimedHitRunner
    {
        ...
        private string combatActionName = "(ninguno)";
        ...
        private PhaseWindows currentPhaseWindow;
        ...
        private void OnGUI()
        {
            ...
            InitStyles();
            windowRect = GUILayout.Window(GetInstanceID(), windowRect, DrawWindow, "Panel Debug Combate");
        }
        ...
        public void RequestAction(BattleActionContext context, Action<BattleSelection> onSelected, Action onCancel)
        {
            ...
            {
                AddLog("Solicitud de accion sin opciones. Cancelando.");
                onCancel?.Invoke();
                return;
            }
            ...
            AddLog($"Esperando seleccion de {context.Player?.DisplayName ?? "Jugador"} ({pendingActions.Count} acciones).");
            AddLog("Tip: usa R/F para ajustar la carga de CP rapidamente.");
        }
        ...
        {
            GUILayout.Label("Estado", headerStyle);
            if (pendingContext != null)
            {
                GUILayout.Label($"Actor: {pendingContext.Player?.DisplayName ?? "(sin nombre)"}", boldLabelStyle);
                GUILayout.Label($"Enemigo: {pendingContext.Enemy?.DisplayName ?? "(ninguno)"}");
            }
            else
            {
                GUILayout.Label("Sin solicitud activa.");
            }
        }

        private void DrawCombatPanel()
        {
            GUILayout.Label("Golpe con timing", headerStyle);
            ...
            GUILayout.Label($"{combatActionName} | Carga {combatCharge}", boldLabelStyle);
            GUILayout.Label($"Phase {Mathf.Clamp(liveCurrentPhase, 1, Mathf.Max(1, liveTotalPhases))}/{Mathf.Max(1, liveTotalPhases)}");
            GUILayout.Label($"Ventana: {combatWindowStart:0.00} - {combatWindowEnd:0.00}");
        ...
        {
            GUILayout.Label("Acciones", headerStyle);

            if (pendingContext == null)
            {
                GUILayout.Label("Sin solicitud.");
            ...
            for (int i = 0; i < pendingActions.Count; i++)
            {
                ...
                if (player != null)
                {
                    ...
                    GUILayout.Label($"Gastara {totalCost} CP (tiene {player.CurrentCP})");
                }

                GUILayout.BeginHorizontal();
                if (GUILayout.Button("Elegir"))
                {
                    desiredCpCharge = Mathf.Clamp(desiredCpCharge, 0, Mathf.RoundToInt(maxCp));
            ...
            GUILayout.Label($"Carga (CP): {desiredCpCharge}", boldLabelStyle);
            desiredCpCharge = Mathf.RoundToInt(GUILayout.HorizontalSlider(desiredCpCharge, 0f, Mathf.Max(0, maxCp)));

            GUILayout.BeginHorizontal();
            if (GUILayout.Button("Cancelar") && pendingOnCancel != null)
            {
                pendingOnCancel?.Invoke();
                AddLog("Solicitud cancelada.");
                ClearPendingRequest();
            }
            GUILayout.EndHorizontal();
        }
        ...
        {
            GUILayout.Label("Practica de timing", headerStyle);

            if (!practiceSession.IsActive)
            {
                GUILayout.Label("Selecciona una accion con timing y presiona Practicar.");
                return;
            }

            GUILayout.Label($"Accion: {practiceSession.Action.id}", boldLabelStyle);
            GUILayout.Label($"Carga CP: {practiceSession.CpCharge}");
            GUILayout.Label($"Fase {practiceSession.PhaseIndex + 1}/{practiceSession.TotalPhases}");
            GUILayout.Label($"Ventana perfecta +- {practiceSession.Tier.PerfectWindowRadius:0.00} en {practiceSession.Tier.PerfectWindowCenter:0.00}");

            Rect barRect = GUILayoutUtility.GetRect(360f, 24f);
            DrawPracticeBar(barRect);

            GUILayout.Label($"Perfecto {practiceSession.PerfectCount} | Bueno {practiceSession.GoodCount} | Fallo {practiceSession.MissCount}");

            GUILayout.BeginHorizontal();
            if (GUILayout.Button(practiceSession.State == PracticeState.Running ? "Detener" : "Reiniciar"))
            {
            ...
            if (practiceSession.State == PracticeState.Completed)
            {
                ...
                GUILayout.Label($"Resultado: Reembolso {summary.CpRefund} | Multiplicador {summary.DamageMultiplier:F2}");
            }
            else
            {
                GUILayout.Label($"Presiona [{timedHitKey}] para golpear.");
            }
        }

        private void DrawLogPanel()
        {
            GUILayout.Label("Registro", headerStyle);
        ...
        private void HandlePlayerActionSelected(BattleSelection selection, int cpBefore)
        {
            ...
            {
                AddLog("> Seleccion cancelada (sin accion).");
                return;
            }

            string actor = ResolveActorName();
            string actionName = ResolveActionName(selection.Action);
            int baseCost = selection.Action.costCP;
            int extra = Mathf.Max(0, selection.CpCharge);
            int total = Mathf.Max(0, baseCost + extra);

            AddLog($"> {actor} eligio {actionName}. CP antes {cpBefore}. CP previsto {total}.");
        }
        ...
        private void HandlePlayerActionResolved(BattleSelection selection, int cpBefore, int cpAfter)
        {
            string actor = ResolveActorName();
            string actionName = ResolveActionName(selection.Action);
            TimedHitResult? timed = selection.TimedHitResult;

            string timingLine = timed.HasValue ? DescribeTiming(timed.Value) : "Sin datos de timing.";
            string damageLine = DescribeDamage(timed);
            string cpLine = DescribeCpDelta(cpBefore, cpAfter);

            AddLog($"> {actor} resolvio {actionName}: {timingLine}");
            if (!string.IsNullOrEmpty(damageLine))
            {
                AddLog(damageLine);
            }
            AddLog(cpLine);
        ...
        {
            return manager?.Player?.DisplayName ?? "Jugador";
        }

        private static string ResolveActionName(BattleActionData action)
        {
            if (action == null)
            {
                return "(accion)";
            ...
            return !string.IsNullOrWhiteSpace(action.id) ? action.id : "(accion)";
        }
        ...
        private static string DescribeTiming(TimedHitResult result)
        {
            if (result.Cancelled)
            {
                return "Timing cancelado.";
            }

            if (result.TotalHits <= 0)
            {
                return "Sin ventanas de timing.";
            }
            ...
            {
                return $"Timing acertado {result.HitsSucceeded}/{result.TotalHits}.";
            }

            return "Se fallo el timing.";
        }

        private static string DescribeDamage(TimedHitResult? result)
        {
            if (!result.HasValue)
            {
                return string.Empty;
            ...
            int damage = result.Value.TotalDamageApplied;
            float mult = result.Value.DamageMultiplier;
            ...
            if (mult >= 1.5f)
            {
                return $"Danio {damage} (critico).";
            }

            if (mult <= 0.75f)
            {
                return $"Danio {damage} (bajo impacto).";
            }

            return $"Danio {damage}.";
        }

        private static string DescribeCpDelta(int before, int after)
        {
            int delta = after - before;
            if (delta < 0)
            {
                return $"CP gastado {Mathf.Abs(delta)}. Quedan {after}.";
            }

            if (delta > 0)
            {
                return $"CP reembolsado {delta}. Quedan {after}.";
            }

            return $"CP sin cambio. Quedan {after}.";
        }

        private void CancelLiveSequence()
        {
            if (liveTcs != null && !liveTcs.Task.IsCompleted)
            {
                FinishTimedHitExecution(null, true);
                var cancelResult = new TimedHitResult(
                    hitsSucceeded: 0,
                    totalHits: Mathf.Max(0, liveTotalPhases),
                    cpRefund: 0,
                    damageMultiplier: 0f,
                    cancelled: true,
                    successStreak: 0);
                liveTcs.TrySetResult(cancelResult);
                OnSequenceCompleted?.Invoke(cancelResult);
            }
        ...
        {
            if (liveCancellation != default)
            {
                liveCancellation.Dispose();
                liveCancellation = default;
            }
            ...
            liveTier = default;
            liveTotalPhases = 0;
            livePhaseDuration = 0f;
            currentPhaseWindow = default;
            combatOverlayActive = false;
            combatPhaseTimer = 0f;
            combatPhaseDuration = 0f;
        }
        ...
        private void StartLivePhase()
        {
            ...
            if (liveCurrentPhase >= liveTotalPhases)
            {
                CompleteLiveSequence();
                return;
            }

            liveCurrentPhase++;
            currentPhaseWindow = ResolveWindows(liveTier);
            livePhaseTimer = 0f;
            liveHoldTimer = 0f;
            livePhaseResolved = false;

            combatCurrentPhase = liveCurrentPhase;
            combatTotalPhases = liveTotalPhases;
            combatWindowStart = currentPhaseWindow.SuccessWindowStart;
            combatWindowEnd = currentPhaseWindow.SuccessWindowEnd;
            ...
            OnPhaseStarted?.Invoke(new TimedHitPhaseInfo(
                liveCurrentPhase,
                liveTotalPhases,
                currentPhaseWindow.SuccessWindowStart,
                currentPhaseWindow.SuccessWindowEnd));
        }

        private void ResolveLivePhase(float normalizedTime, bool autoMiss)
        {
            if (liveTcs == null || liveTcs.Task.IsCompleted || livePhaseResolved)
            {
                return;
            }

            var outcome = EvaluateOutcome(liveTier, currentPhaseWindow, normalizedTime, autoMiss);

            switch (outcome.Kind)
            {
                case TimedOutcome.Perfect:
                    livePerfectCount++;
                    break;
                case TimedOutcome.Good:
                    liveGoodCount++;
                    break;
                default:
                    liveMissCount++;
                    liveForceMisses = true;
                    break;
            }

            bool success = outcome.Kind != TimedOutcome.Miss;
            ...
            combatLastOutcome = BuildOutcomeLabel(outcome.Kind, autoMiss, outcome.Accuracy);
            combatPhaseTimer = combatPhaseDuration;
            ...
            livePhaseResolved = true;
            liveHoldTimer = 0f;
        }

        private void AutoResolveRemaining()
        {
            if (liveTcs == null || liveTcs.Task.IsCompleted)
            {
                ResetLiveState();
            ...
            // Continue resolving remaining phases as misses without waiting for input.
            while (liveCurrentPhase < liveTotalPhases)
            {
                StartLivePhase();
                if (liveMode != TimedSequenceMode.Execute || liveTcs == null || liveTcs.Task.IsCompleted)
                {
                    return;
                }
        ...
        private void CompleteLiveSequence()
        {
            if (liveTcs == null || liveTcs.Task.IsCompleted)
            {
                ResetLiveState();
            ...
            var result = BuildLiveResult();
            FinishTimedHitExecution(result, false);
            liveTcs.TrySetResult(result);
            OnSequenceCompleted?.Invoke(result);

            ResetLiveState();
        }

        private TimedHitResult BuildLiveResult()
        {
            int totalHits = Mathf.Max(0, livePerfectCount + liveGoodCount + liveMissCount);
            int hitsSucceeded = Mathf.Max(0, livePerfectCount + liveGoodCount);
            int refund = Mathf.Clamp(hitsSucceeded, 0, liveTier.RefundMax);
            float multiplier = CalculateCombinedMultiplier(liveTier, livePerfectCount, liveGoodCount);
        ...
        private static float CalculateCombinedMultiplier(Ks1TimedHitProfile.Tier tier, int perfectCount, int goodCount)
        {
        ...
        private static LivePhaseOutcome EvaluateOutcome(
            ...
            bool autoMiss)
        {
            ...
            if (autoMiss)
            {
                ...
                return new LivePhaseOutcome(TimedOutcome.Miss, missMultiplier, 0f);
            }

            float delta = Mathf.Abs(normalizedTime - window.PerfectCenter);
            float successRadius = window.SuccessRadius > 0f ? window.SuccessRadius : 1f;
            float accuracy = Mathf.Clamp01(1f - (delta / successRadius));

            if (delta <= window.PerfectRadius)
            {
                float perfectMultiplier = tier.PerfectHitMultiplier > 0f ? tier.PerfectHitMultiplier : 1.5f;
                return new LivePhaseOutcome(TimedOutcome.Perfect, perfectMultiplier, accuracy);
            }

            if (delta <= window.SuccessRadius)
            {
                float successMultiplier = tier.SuccessHitMultiplier > 0f ? tier.SuccessHitMultiplier : 1f;
                return new LivePhaseOutcome(TimedOutcome.Good, successMultiplier, accuracy);
            }

            float missMult = tier.MissHitMultiplier > 0f ? tier.MissHitMultiplier : 0f;
            return new LivePhaseOutcome(TimedOutcome.Miss, missMult, 0f);
        }
        ...
        private static string BuildOutcomeLabel(TimedOutcome outcome, bool autoMiss, float accuracy)
        {
            switch (outcome)
            {
                case TimedOutcome.Perfect:
                    return "Perfecto!";
                case TimedOutcome.Good:
                    int pct = Mathf.RoundToInt(accuracy * 100f);
                    return $"Buen timing ({pct}%).";
                default:
                    return autoMiss ? "Se agoto la ventana." : "Fallo.";
            }
        }

        private readonly struct LivePhaseOutcome
        {
            public LivePhaseOutcome(TimedOutcome kind, float multiplier, float accuracy)
            {
                Kind = kind;
        ...
        public async Task<TimedHitResult> RunAsync(TimedHitRequest request)
        {
            ...
            combatActionName = request.ActionData?.displayName ?? request.ActionData?.id ?? "(desconocido)";
            ...
            currentPhaseWindow = default;
            combatTotalPhases = liveTotalPhases;
            combatCurrentPhase = 0;

            combatStatus = $"Timing en curso ({request.ActionData?.displayName ?? request.ActionData?.id ?? "(desconocido)"})";
            combatLastOutcome = "Esperando entrada...";
        ...
        private void FinishTimedHitExecution(TimedHitResult? result, bool cancelled)
        {
            if (cancelled)
            {
                combatStatus = "Timing cancelado.";
            }
            else if (result.HasValue)
            {
                var value = result.Value;
                int total = Mathf.Max(1, value.TotalHits);
                combatStatus = $"Resultado timing: {value.HitsSucceeded}/{total} aciertos, reembolsa {value.CpRefund}";
            }
            else
            {
                combatStatus = "Timing resuelto.";
            }
        }
        ...
        private void ConfirmSelection(ActionEntry entry)
        {
            ...
            AddLog($"Selecciono {entry.Action.id} con CP {cp}.");
        ...
        private void StartPractice(ActionEntry entry, int cpCharge)
        {
            if (entry.TimedProfile == null)
            {
                AddLog($"La accion {entry.Action.id} no tiene perfil de timing.");
                return;
            }
            ...
            if (!practiceSession.IsActive)
            {
                AddLog("No se puede iniciar practica (perfil sin golpes).");
            }
            else
            {
                AddLog($"Practica iniciada para {entry.Action.id}. Presiona [{timedHitKey}] para golpear.");
            }
        }
