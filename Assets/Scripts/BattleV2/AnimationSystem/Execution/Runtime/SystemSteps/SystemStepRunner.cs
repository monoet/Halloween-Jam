...
using BattleV2.Core;
using Scheduler = BattleV2.AnimationSystem.Execution.Runtime.StepScheduler;
...
namespace BattleV2.AnimationSystem.Execution.Runtime.SystemSteps
{
    internal sealed class SystemStepRunner
    {
        ...
        internal bool TryHandle(ActionStep step, StepSchedulerContext context, Scheduler.ExecutionState state, out Scheduler.StepResult result)
        {
            result = Scheduler.StepResult.Skipped;
            ...
            switch (executorId.ToLowerInvariant())
            {
                case SystemStepWindowOpen:
                    ...
                    result = Scheduler.StepResult.Completed;
                    return true;
        ...
        private void HandleWindowOpen(ActionStep step, StepSchedulerContext context, Scheduler.ExecutionState state)
        {
            ...
            state.RegisterWindow(id, new Scheduler.ExecutionState.WindowState(id, tag));
            context.EventBus?.Publish(new AnimationWindowEvent(context.Actor, tag, payload, start, end, true, index, count));
        }

        private void HandleWindowClose(ActionStep step, StepSchedulerContext context, Scheduler.ExecutionState state)
        {
            var parameters = step.Parameters;
            if (!TryGetRequired(parameters, "id", out var id))
            {
            ...
            Scheduler.ExecutionState.WindowState window = null;
        ...
        private Scheduler.StepResult HandleGate(ActionStep step, StepSchedulerContext context, Scheduler.ExecutionState state)
        {
            var parameters = step.Parameters;
            if (!TryGetRequired(parameters, "id", out var id))
            {
                BattleLogger.Warn(logTag, "gate.on missing 'id'.");
                return Scheduler.StepResult.Failed;
            }
            ...
            if (!state.TryGetWindowResult(id, out var hitResult))
            {
                ...
                {
                    return Scheduler.StepResult.Completed;
                }

                bool abortOnTimeout = parameters.TryGetBool("abortOnTimeout", out var abortTimeout) && abortTimeout;
                return abortOnTimeout ? Scheduler.StepResult.Abort("GateTimeout") : Scheduler.StepResult.Branch(timeoutLabel);
            }
            ...
            if (abort)
            {
                return Scheduler.StepResult.Abort(isSuccess ? "GateAbortSuccess" : "GateAbortFail");
            }

            return Scheduler.StepResult.Branch(branchTarget);
        }
        ...
        private Scheduler.StepResult HandleFallback(ActionStep step, StepSchedulerContext context)
        {
            ...
            return Scheduler.StepResult.Abort(reason);
        }
